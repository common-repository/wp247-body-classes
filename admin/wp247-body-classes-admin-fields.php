...
*/
	...
	foreach ( $GLOBALS as $key => $value ) {
		if ( 'is_' == substr( $key, 0, 3 ) ) {
			...
			$globals_options[ 'is-' . $slug ] = '<span class="wp247sapi">is-'.$slug.'</span><span>$GLOBALS[ ' . $key . ' ] is set.</span>';
			$globals_options[ 'is-not-' . $slug ] = '<span class="wp247sapi">is-not-'.$slug.'</span><span>$GLOBALS[ ' . $key . ' ] is not set.</span>';
		}
	}
...
*/
		...
		'wp247_body_classes_environment' => array(
			...
			array(
				'name' => 'GLOBALS',
				'label' => __( '$GLOBALS', 'wp247-body-classes' ),
				'intro' => __( '<b>$GLOBALS</b><br />These calsses indicate if the selected $GLOBALS variable option is or is not set.', 'wp247-body-classes' ),
				'type' => 'multicheck',
				'options' => $globals_options
			),
		),
		...
		'wp247_body_classes_archive' => array(
			array(
				...
				'type' => 'multicheck',
				'options' => wp247_body_classes_admin_fields_multipage( 'is-search', 'is-not-search' )
			),
			array(
				'name' => 'archive',
				...
				'type' => 'multicheck',
				'options' =>wp247_body_classes_admin_fields_multipage( 'is-archive', 'is-not-archive' )
			),
			array(
				'name' => 'author',
				...
				'type' => 'multicheck',
				'options' => wp247_body_classes_admin_fields_multipage(
								array(
									'is-author' => 'is-author',
		...
		'wp247_body_classes_post' => array(
			array(
				...
				'type' => 'multicheck',
				'options' => wp247_body_classes_admin_fields_multipage(	'is-single', 'is-not-single' )
			),
			array(
				'name' => 'singular',
				'label' => __( 'Singular', 'wp247-body-classes' ),
				'intro' => __( '<b>is_singular()</b><br/>This class indicates if a single post of any post type (including attachment and page post types) is being displayed.', 'wp247-body-classes' ),
				'type' => 'multicheck',
				'options' => wp247_body_classes_admin_fields_multipage(	'is-singular', 'is-not-singular' )
			),
			array(
				'name' => 'sticky',
				...
				'type' => 'multicheck',
				'options' => wp247_body_classes_admin_fields_multipage( 'is-sticky', 'is-not-sticky' )
			),
		),
	...
	foreach ( $ptypes as $pt )
	{
		...
		$xintro = '';
		$opts = wp247_body_classes_admin_fields_multipage( 'is-'.$cleanpt, 'is-not-'.$cleanpt );
		if ( in_array( $pt, [ 'page', 'post' ] ) ) {
			$xintro = ' If a <b>'.$ucwpt.'</b> post type is being displayed, additional body classes providing metadata about the <b>'.$ucwpt.'</b> being displayed may optionally be included.';
		}
		if ( 'page' == $pt ) {
			$opts = wp247_body_classes_admin_fields_multipage(
						array(
							'is-'.$pt => 'is-'.$pt,
...
function wp247_body_classes_admin_fields_multipage( $before, $after ) {
	$fields = [];
	foreach ( ( is_array( $before ) ? $before : [ $before => $before ] ) as $key => $desc ) $fields[ $key ] = $desc;
	$fields[ 'is-multipage' ] = '<span class="wp247sapi-indent">is-multipage</span>';
	$fields[ 'is-multipage-page-count' ] = '<span class="wp247sapi-indent">is-multipage-pages-{count}</span><span>(example: is-multipage-pages-12)</span>';
	$fields[ 'is-multipage-page-number' ] = '<span class="wp247sapi-indent">is-multipage-page-{number}</span><span>(example: is-multipage-page-8)</span>';
	$fields[ 'is-multipage-page-first' ] = '<span class="wp247sapi-indent">is-multipage-page-first</span>';
	$fields[ 'is-multipage-page-last' ] = '<span class="wp247sapi-indent">is-multipage-page-last</span>';
	$fields[ 'is-not-multipage' ] = '<span class="wp247sapi-indent">is-not-multipage</span>';
	foreach ( ( is_array( $after ) ? $after : [ $after => $after ] ) as $key => $desc ) $fields[ $key ] = $desc;
	return $fields;
}
?>