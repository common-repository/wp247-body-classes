...
if ( !defined( 'WP247_BODY_CLASSES_VERSION' ) )
{
	...
	define( 'WP247_BODY_CLASSES_VERSION', '2.2.1' );
	...
	global $wp247_body_classes_multipage_options;
	...
	add_action( 'wp_footer', 'wp247_body_classes_do_action_wp_footer', 99999 );
	...
	function wp247_body_classes_do_action_body_class( $classes )
	{
		...
		if ( $option_groups [ 'wp247_body_classes_environment' ] ) {
			...
			foreach ( $GLOBALS as $key => $value ) {
				if ( 'is_' == substr( $key, 0, 3 ) ) {
					$slug = str_replace( '_', '-', substr( $key, 3 ) );
					$globals_options[] = array( $slug, ( '1' == $value ) );
				}
			}
...
// If Page or Post, do Categories and Tags if required
			...
			{
				$class_driver[] = array( str_replace( '_', '-', $pt ), $pt == $post_type );
			}
		...
		global $wp247_body_classes_multipage_options;
		$wp247_body_classes_multipage_options = [];

		foreach ( $class_driver as $cd )
		{
//			if ( !is_array( $cd ) ) continue; // TODO: Figure out where non-array value iscomming from
			$option = $cd[ 0 ];
			$is_value = $cd[ 1 ];
			...
			if ( $is_value )
			{
				...
				$wp247_body_classes_multipage_options = array_merge( $wp247_body_classes_multipage_options
																	,array_intersect_key( ( isset( $options[ $option ] ) and is_array( $options[ $option ] ) )
																			? $options[ $option ]
																			: []
																		,[
																			 'is-multipage'				=> ''
																			,'is-multipage-page-count'	=> ''
																			,'is-multipage-page-number'	=> ''
																			,'is-multipage-page-first'	=> ''
																			,'is-multipage-page-last'	=> ''
																			,'is-not-multipage'			=> ''
																		] ) );
			}
		...
		// Add is-multipage Body Classes for Archive Classes
		if ( !empty( $wp247_body_classes_multipage_options ) ) {
			global $wp_query;
			$page		= is_singular() ? $wp_query->query_vars['page'] : $wp_query->query_vars['paged'];
			$page		= max( 1, $page );
			$numpages	= is_singular() ? substr_count( $wp_query->post->post_content, '<!--nextpage-->' ) + 1 : $wp_query->max_num_pages;
			if ( $numpages > 1 ) {
				if ( isset( $wp247_body_classes_multipage_options[ 'is-multipage' ] ) ) $classes[] = 'is-multipage';
				if ( isset( $wp247_body_classes_multipage_options[ 'is-multipage-page-count' ] ) ) $classes[] = 'is-multipage-pages-' . $numpages;
				if ( isset( $wp247_body_classes_multipage_options[ 'is-multipage-page-number' ] ) ) $classes[] .= 'is-multipage-page-' . $page;
				if ( isset( $wp247_body_classes_multipage_options[ 'is-multipage-page-first' ] ) and 1 == $page ) $classes[] = 'is-multipage-page-first';
				if ( isset( $wp247_body_classes_multipage_options[ 'is-multipage-page-last' ] ) and $page == $numpages ) $classes[] = 'is-multipage-page-last';
			}
			elseif ( isset( $wp247_body_classes_multipage_options[ 'is-not-multipage' ] ) ) $classes[] = 'is-not-multipage';
		}
	...
	// Dump the debug array into the footer
	function wp247_body_classes_do_action_wp_footer()
	{

	if ( WP247_BODY_CLASSES_DEBUG )
		{
			if ( !empty( $wp247_body_classes_debug ) ) {
//				echo PHP_EOL.PHP_EOL.'<!-- -------------------- wp247_body_classes_debug --------------------'.PHP_EOL.PHP_EOL;
				echo PHP_EOL.PHP_EOL.'<pre>'.PHP_EOL.'-------------------- wp247_body_classes_debug --------------------'.PHP_EOL.PHP_EOL;
				var_dump( $wp247_body_classes_debug );
//				echo PHP_EOL.PHP_EOL.' -------------------- wp247_body_classes_debug -------------------- -->'.PHP_EOL.PHP_EOL;
				echo PHP_EOL.PHP_EOL.'-------------------- wp247_body_classes_debug --------------------'.PHP_EOL.'</pre>'.PHP_EOL.PHP_EOL;
			}
		}
	}
	...
	function wp247_body_classes_get_available_mobile_detect_versions()
	{
		...
		uksort( $mdvers, 'strnatcasecmp' );
		$mdvers = array_reverse( $mdvers, true );
		return $mdvers;
	}
	...
	{
//die("wp247_body_classes_mobile_detect_path: ".WP247_BODY_CLASSES_PLUGIN_PATH . wp247_body_classes_get_mobile_detect_version() . '/Mobile_Detect.php');
		require_once WP247_BODY_CLASSES_PLUGIN_PATH . wp247_body_classes_get_mobile_detect_version() . '/Mobile_Detect.php';
		global $wp247_mobile_detect;
		$wp247_mobile_detect = null;
		if ( class_exists( 'WP247_Mobile_Detect' ) ) $wp247_mobile_detect = new WP247_Mobile_Detect();	// Pre-version 3.74
		elseif ( class_exists( 'Detection\WP247_Mobile_Detect' ) ) $wp247_mobile_detect = new Detection\WP247_Mobile_Detect();
	}
